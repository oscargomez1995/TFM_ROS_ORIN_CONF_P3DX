...
def generate_launch_description():
    ...
    xacro_path = os.path.join(get_package_share_directory(
        'p3dx_description'), 'urdf', 'p3dx.xacro')
    ...
    rviz = Node(
        ...
        output='screen',
        arguments=[
            '-d', os.path.join(get_package_share_directory('p3dx_description'), 'rviz', 'custom_setup.rviz')]
    )

    return LaunchDescription([
        DeclareLaunchArgument('use_sim_time', default_value='false'),
        DeclareLaunchArgument('robot_description', default_value=doc.toxml()),
        robot_joint_publisher, robot_state_publisher, rviz
    ])
